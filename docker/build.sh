...
docker build -f ${CURRENT_DIR}/docker/Dockerfile --target=${TARGET} -t ${PREFIX}common-build-agent-dev:$(cat docker/Dockerfile.version) -t ${PREFIX}common-build-agent-dev:latest .
